...
export function InstagramPostForm() {
    ...
    const aestheticOptions = ["Minimalist", "Vibrant", "Moody", "Cinematic", "Vintage", "Dreamy"];
    ...
    return (
        <form onSubmit={handleSubmit} className="w-full max-w-3xl mx-auto bg-white/60 backdrop-blur-xl border border-white/40 shadow-xl rounded-2xl p-8 flex flex-col gap-6">
            {/* Field 1: Visual Concept */}
            <div className="flex flex-col gap-2">
                <label htmlFor="topic" className="text-sm font-semibold text-slate-700 ml-1">
                    Visual Concept / Core Idea <span className="text-rose-500">*</span>
                </label>
                <input
                    id="topic"
                    type="text"
                    ...
                    placeholder="e.g., A behind-the-scenes look at our creative process"
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white/50 text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#FF0069]/50 transition-all font-medium text-lg"
                />
            </div>

            {/* Field 2: Aesthetic / Vibe */}
            <div className="flex flex-col gap-2">
                <label className="text-sm font-semibold text-slate-700 ml-1">
                    Aesthetic / Vibe <span className="text-slate-400 font-normal">(Optional)</span>
                </label>
                <div className="flex flex-wrap gap-2">
                    {aestheticOptions.map((vibe) => (
                        <button
                            ...
                            className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${localAesthetic === vibe ? 'bg-[#FF0069] text-white shadow-md shadow-[#FF0069]/20 border-[#FF0069]' : 'bg-white/50 text-slate-600 border border-slate-200 hover:bg-white hover:border-[#FF0069]/40'}`}
                        >
                            {vibe}
                        </button>
                    ))}
            ...
            <div className="flex flex-col gap-2 relative">
                <div className="flex items-center justify-between ml-1">
                    ...
                    <button
                        ...
                        className="flex items-center gap-1.5 text-xs font-semibold text-[#FF0069] bg-[#FF0069]/10 py-1 px-3 rounded-full hover:bg-[#FF0069]/20 transition-colors disabled:opacity-50"
                    >
                ...
                <textarea
                    ...
                    placeholder="Any specific text you want in the caption? Leave blank for AI to generate."
                    rows={4}
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white/50 text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#FF0069]/50 transition-all resize-none"
                />
            </div>
            ...
            <div className="flex flex-col gap-3">
                ...
                <div 
                    ...
                    className="flex flex-col items-center justify-center w-full min-h-[120px] border-2 border-dashed border-slate-300 rounded-xl cursor-pointer bg-slate-50/50 hover:bg-[#FF0069]/5 hover:border-[#FF0069]/40 transition-colors py-6"
                >
                    <UploadCloud className="w-10 h-10 mb-2 text-[#FF0069]/70" />
                    <p className="mb-1 text-sm text-slate-600"><span className="font-semibold text-[#FF0069]">Click to upload</span> or drag and drop</p>
                ...
                <input
                    ...
                    placeholder="Instructions for AI regarding these images (e.g., 'Emphasize the lighting')"
                    className="w-full mt-2 px-4 py-2.5 text-sm rounded-lg border border-slate-200 bg-white/50 text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#FF0069]/50 transition-all"
                />

                <label className="flex items-center gap-2 mt-1 cursor-pointer group w-max">
                    <div className={`w-5 h-5 rounded flex items-center justify-center border transition-colors ${localIncludeMedia ? 'bg-[#FF0069] border-[#FF0069]' : 'bg-white border-slate-300 group-hover:border-[#FF0069]/60'}`}>
            ...
            <div className="flex flex-col items-center mt-2 w-full">
                <motion.button
                    ...
                    className={`w-full flex items-center justify-center gap-2 px-6 py-4 rounded-xl text-white font-bold text-lg shadow-lg transition-all ${!localTopic.trim() || isSubmitting ? 'bg-slate-300 shadow-none cursor-not-allowed text-slate-500' : 'bg-gradient-to-r from-[#FF0069] to-pink-500 hover:shadow-[#FF0069]/30 shadow-[#FF0069]/20'}`}
                >
                    {isSubmitting ? (
